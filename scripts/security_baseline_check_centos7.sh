...
# echo -e "\033[32m\n请使用命令 sh security_baseline_check_centos7.sh > /var/log/security_baseline_check.txt 执行脚本 \n\033[0m"
echo > /var/log/security_baseline_check.txt
echo -e "\033[32m当前主机名：$(hostname)\033[0m" >> /var/log/security_baseline_check.txt
echo -e "\033[32m主机IP地址：\n$(ifconfig  | grep "inet" | grep -v 'inet6' | grep -v '127.0.0.1' | awk '{print $2}')\033[0m" >> /var/log/security_baseline_check.txt
echo -e "\033[32m当前CentOS版本：$(cat /etc/redhat-release | awk '{print $4}')\033[0m" >> /var/log/security_baseline_check.txt
	...
	for service in $@;do
		...
		if [[ $(systemctl list-unit-files | grep $service) != '' ]];then
			...
			if [[ ${is_active} == 'active' ]];then
				echo -e "\033[31m✘ $service Is-Actice:$is_active Is-Enabled:$is_enabled \033[0m" >> /var/log/security_baseline_check.txt
			else
				echo -e "\033[32m√ $service Is-Actice:$is_active Is-Enabled:$is_enabled \033[0m" >> /var/log/security_baseline_check.txt
			fi
		else
...
if [[ $(systemctl is-active firewalld) == 'active' ]];then
	echo -e "\033[31m✘ Firewalld正在运行 \033[0m" >> /var/log/security_baseline_check.txt
else
	echo -e "\033[32m√ Firewalld未启动\033[0m" >> /var/log/security_baseline_check.txt
fi
