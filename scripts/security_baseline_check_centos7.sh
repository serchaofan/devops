	...
	for service in $@;do
		...
		if [[ $(systemctl list-unit-files | grep $service) != '' ]];then
			...
			if [[ ${is_active} == 'active' ]];then
				echo -e "\033[31m✘ $service Is-Actice:$is_active Is-Enabled:$is_enabled \033[0m"
			else
				echo -e "\033[32m√ $service Is-Actice:$is_active Is-Enabled:$is_enabled \033[0m"
			fi
		else
...
if [[ $(systemctl is-active firewalld) == 'active' ]];then
	echo -e "\033[31m✘ Firewalld正在运行 \033[0m"
else
	echo -e "\033[32m√ Firewalld未启动\033[0m"
fi
